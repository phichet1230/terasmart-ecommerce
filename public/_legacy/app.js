...
class TeraSmartApp {
  ...
  setupEventListeners() {
    ...
    this.setupAddressAutocompleteListeners();
  }
  ...
  switchTab(tabName) {
    ...
    if (tabName === 'catalog') {
      ...
      const promoBtn = document.querySelector('.promo-code-container button');
      if (promoBtn) {
        promoBtn.innerText = 'ใช้งาน';
  ...
  async applyPromoCode() {
    ...
    const msgEl = document.getElementById('checkout-promo-message');
    const btn = document.querySelector('.promo-code-container button');
    ...
    // Calculate subtotal of current cart/buy-now items
    let subtotal = 0;
    if (this.checkoutMode === 'buy_now' && this.buyNowItem) {
      subtotal = parseFloat(this.buyNowItem.price) * this.buyNowItem.quantity;
    } else {
      const selectedItems = this.cartItems.filter(item => item.selected);
  ...
  removePromoCode() {
    ...
    if (btn) {
      btn.innerText = 'ใช้งาน';
      btn.className = 'btn btn-secondary';
    }

    this.showToast('ยกเลิกคูปองส่วนลดแล้ว');
    this.renderCheckout();
  }
  ...
  async loadAddressDb() {
    try {
      const res = await fetch('/thailand_addresses.json');
      if (res.ok) {
        this.addressDb = await res.json();
        console.log('Loaded Thailand Address Database:', this.addressDb.length, 'provinces');
        this.populateProvincesDatalist();
      }
    } catch (err) {
      console.error('Failed to load Thailand address database:', err);
    }
  }

  populateProvincesDatalist() {
    const listEl = document.getElementById('provinces-list');
    if (!listEl) return;
    listEl.innerHTML = this.addressDb
      .map(prov => `<option value="${prov.name_th}"></option>`)
      .join('');
  }

  setupAddressAutocompleteListeners() {
    const provinceInput = document.getElementById('address-province');
    ...
    if (!provinceInput || !districtInput || !subdistrictInput || !postalInput) return;

    let lastProvince = provinceInput.value.trim();
    let lastDistrict = districtInput.value.trim();

    // 1. Province Input Listener
    provinceInput.addEventListener('input', () => {
      const currentProv = provinceInput.value.trim();
      
      // If province is completely cleared, clear child fields
      if (currentProv === '') {
        districtInput.value = '';
      ...
      const provMatch = this.addressDb.find(p => p.name_th === currentProv);
      if (provMatch) {
        // Only clear children if the province has actually changed to a different valid one
        if (currentProv !== lastProvince) {
          districtInput.value = '';
          subdistrictInput.value = '';
          postalInput.value = '';
          lastProvince = currentProv;
          lastDistrict = '';
        }

        const districtList = document.getElementById('districts-list');
        if (districtList) {
          districtList.innerHTML = provMatch.districts
            .map(dist => `<option value="${dist.name_th}"></option>`)
            .join('');
        }
      }
    });

    // 2. District Input Listener
    districtInput.addEventListener('input', () => {
      const currentProv = provinceInput.value.trim();
      const currentDist = districtInput.value.trim();
      ...
      const distMatch = provMatch.districts.find(d => d.name_th === currentDist);
      if (distMatch) {
        // Only clear subdistrict if the district has changed to a different valid one
        if (currentDist !== lastDistrict) {
          subdistrictInput.value = '';
          postalInput.value = '';
          lastDistrict = currentDist;
        }

        const subdistrictList = document.getElementById('subdistricts-list');
        if (subdistrictList) {
          subdistrictList.innerHTML = distMatch.sub_districts
            .map(sub => `<option value="${sub.name_th}"></option>`)
            .join('');
        }
      }
    });

    // 3. Sub-district Input Listener
    subdistrictInput.addEventListener('input', () => {
      const currentProv = provinceInput.value.trim();
      const currentDist = districtInput.value.trim();
      const currentSub = subdistrictInput.value.trim();
      ...
      const subMatch = distMatch.sub_districts.find(s => s.name_th === currentSub);
      if (subMatch) {
        postalInput.value = subMatch.zip_code;
      }
    });
  }
}
