...
async function main() {
  try {
    ...
    // Drop all tables if they exist to allow clean recreate
    console.log('Dropping existing tables...');
    await client.query('DROP TABLE IF EXISTS coupons, payment_methods, cart_items, carts, audit_logs, shipping, payments, order_items, orders, addresses, flash_sales, product_variants, products, categories, password_resets, refresh_tokens, users CASCADE');
    ...
    // Clean out SELECT queries if they are run directly
    ...
    // 2. Clear old test data if any to start fresh
    console.log('Clearing old data...');
    await client.query('TRUNCATE refresh_tokens, cart_items, carts, audit_logs, shipping, payments, order_items, orders, addresses, flash_sales, product_variants, products, categories, users CASCADE');

    // 3. Seed users (admin and customer)
    ...
    // Product 1: Tera Phone 15 (Gadgets)
    ...
    // Product 2: Tera Watch v2 (Wearables)
    ...
    // Product 3: Tera Buds Pro (Accessories)
    ...
    // Product 4: Tera PowerBank 20k (Accessories)
