...
/**
 * 5. EasySlip Developer API Integration Helper (v1 / v2 Support)
 * เชื่อมต่อ EasySlip API ตรวจสอบสลิปผ่านระบบธนาคารโดยตรง (ถ้ามี EASYSLIP_API_KEY ใน .env)
 */
async function verifyWithEasySlip({ filePath, qrPayload }) {
  const apiKey = process.env.EASYSLIP_API_KEY;
  if (!apiKey) {
    return { isConfigured: false, message: 'ไม่ได้ตั้งค่า EASYSLIP_API_KEY ในระบบ' };
  }
  ...
  try {
    ...
    if (qrPayload) {
      // 1. ลองเรียก v1/verify ด้วย payload หากมี QR Payload สแกนได้
      response = await fetch(`https://api.easyslip.com/v1/verify?payload=${encodeURIComponent(qrPayload)}`, {
        method: 'GET',
        headers: {
      ...
      // 2. เรียก v2 หรือ v1 ด้วยรูปภาพ
      const fileBuffer = fs.readFileSync(filePath);
      const boundary = `----EasySlipBoundary${Date.now()}`;
      ...
      response = await fetch('https://api.easyslip.com/v2/verify/bank/image', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${apiKey}`,
    ...
    if (!response) {
      return { isConfigured: true, success: false, message: 'ไม่มีข้อมูลสลิปสำหรับส่งตรวจสอบ' };
    }
    ...
    if (response.ok && (result.status === 200 || result.status === 'success')) {
      ...
      return {
        isConfigured: true,
        success: true,
        amount: data.amount ? parseFloat(data.amount) : (data.transAmount ? parseFloat(data.transAmount) : null),
        transRef: data.transRef || data.transactionId || data.transId || null,
        sendingBank: data.sender?.bank?.name || data.sendingBank || 'EasySlip Verified Bank',
        senderName: data.sender?.account?.name?.th || data.senderName || 'ผู้โอนเงินผ่านระบบธนาคาร',
        receiverName: data.receiver?.account?.name?.th || data.receiverName || '',
        transDate: data.date ? new Date(data.date) : new Date(),
        raw: data
      };
    } else {
      return {
        isConfigured: true,
        success: false,
        errorCode: result.message || result.error || 'easyslip_error',
        message: result.message || 'EasySlip ตรวจสอบสลิปไม่สำเร็จ'
      };
    }
  } catch (err) {
    console.warn('EasySlip API Warning:', err.message);
    return { isConfigured: true, success: false, error: err.message };
  }
}
