...
export default function Storefront() {
  ...
  // Catalog States
  const [products, setProducts] = useState<Product[]>(() => defaultStorefrontProducts);
  ...
  }, [activeTab, createdOrderId, verifiedPaymentInfo, createdOrderTotal]);
  ...
  const handleRequestForgotToken = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(res.message || 'ส่งรหัส OTP 6 หลักไปยังอีเมลของคุณเรียบร้อยแล้ว กรุณาตรวจสอบกล่องจดหมาย');
      setForgotToken(''); // Explicitly keep field blank so user enters PIN manually from email
      setForgotStep('reset');
    } catch (err: any) {
